...
import { SystemStats, AnalyticsSummary, Message, Project, Service, Skill, User } from '../types';
    ...
    <div className="space-y-4">
      ...
      >
        <form onSubmit={submit} className="space-y-3">
          ...
          <Field label="Dot color"><input type="color" className="input-field h-10 w-full p-1 bg-slate-850 border border-slate-700/60 rounded" value={form.color} onChange={set('color')} /></Field>
        </form>
      </Modal>
    </div>
  );
...
function SysCard({ label, value, sub, percent, color = 'blue' }: SysCardProps) {
  const bar: Record<string, string> = { blue: 'bg-blue-500', purple: 'bg-purple-500', emerald: 'bg-emerald-500' };
  return (
    <div className="card p-5">
      <p className="text-xs text-slate-500 uppercase tracking-wider mb-1">{label}</p>
      <p className="text-2xl font-bold text-slate-100 mb-1">{value}</p>
      {sub && <p className="text-xs text-slate-400 font-mono truncate">{sub}</p>}
      {percent !== undefined && (
        <div className="mt-3">
...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    loadSystem();
    // Poll stats every 5 seconds for real-time RPi5 server health
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100">
      <Navbar />
      <main className="mx-auto max-w-7xl px-4 sm:px-6 pt-28 pb-12">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          <div>
            <h1 className="text-3xl font-bold gradient-text">Dashboard</h1>
            <p className="text-slate-500 text-sm mt-1">Host system health & control center</p>
          </div>
          <div className="flex flex-wrap gap-2">
            <Link to="/files" className="btn-primary py-2 px-4 text-xs font-semibold flex items-center gap-1.5">
              📂 Files
            </Link>
            <Link to="/clipboard" className="btn-outline py-2 px-4 text-xs font-semibold flex items-center gap-1.5">
              📋 Clipboard
            </Link>
            <Link to="/terminal" className="btn-outline py-2 px-4 text-xs font-semibold flex items-center gap-1.5">
              ⌨️ Terminal
            </Link>
          </div>
        </div>

        {/* Host Stats & Tools Header Grid */}
        {loadingStats ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            <SysCard label="Host CPU" value={`${stats.cpu.load}%`} sub={stats.cpu.model} percent={parseFloat(stats.cpu.load)} color="blue" />
            <SysCard label="Host Memory" value={stats.memory.used} sub={`of ${stats.memory.total}`} percent={stats.memory.percent} color="purple" />
            <SysCard label="Host Disk" value={stats.disk.used} sub={`of ${stats.disk.total}`} percent={stats.disk.percent} color="emerald" />
            <div className="card p-5 flex flex-col justify-between">
              <div>
                <p className="text-xs text-slate-500 uppercase tracking-wider mb-1">Temperature & Uptime</p>
                <p className="text-xl font-bold text-slate-100 mb-0.5">{stats.temperature}</p>
                <p className="text-xs text-slate-500">{stats.uptime}</p>
              </div>
              <div className="flex gap-2.5 mt-2 pt-2 border-t border-slate-800/50">
                <Link to="/terminal" className="text-xs font-semibold text-cyan-400 hover:text-cyan-300">Terminal →</Link>
                <Link to="/files" className="text-xs font-semibold text-cyan-400 hover:text-cyan-300">Files →</Link>
                <Link to="/clipboard" className="text-xs font-semibold text-cyan-400 hover:text-cyan-300">Clips →</Link>
              </div>
            </div>
          </div>
        )}
        ...
        <nav className="flex flex-wrap gap-1 border-b border-slate-700 mb-6 pb-2">
          {TABS.map((t) => (
            <button
              ...
              className={`px-4 py-2 rounded-t text-sm font-medium transition-all ${tab === t.id ? 'bg-slate-800 text-cyan-300 ring-1 ring-cyan-500/40' : 'text-slate-400 hover:text-slate-200 hover:bg-slate-800/50'}`}
            >
              {t.label}
