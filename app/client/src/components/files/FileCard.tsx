...
import { Download, Trash2, FileText, Image, Video, Music, FileCode, Archive, File, Pin, PinOff } from 'lucide-react';
...
function fileTypeStyle(mime: string): { icon: React.ReactNode; bg: string; accent: string } {
...
export default function FileCard({ file, view, selected, checked, onSelect, onDoubleClick, onDelete, onPin, onCheck }: Props) {
  ...
  const { icon, bg, accent } = fileTypeStyle(file.mime_type ?? '');
    ...
    return (
      <div
        ...
        className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all cursor-grab active:cursor-grabbing group select-none
          ${checked ? 'bg-cyan-500/10 border border-cyan-500/40' : selected ? 'bg-cyan-500/8 border border-cyan-500/20' : 'hover:bg-white/4 border border-transparent'}
      ...
      >
        ...
        <div className={`w-8 h-8 rounded-md bg-gradient-to-br ${bg} border ${accent} flex items-center justify-center shrink-0`}>
          <span className={accent.split(' ')[0]}>{icon}</span>
        </div>

        <div className="flex-1 min-w-0">
          <p className={`text-sm truncate ${checked || selected ? 'text-cyan-200' : 'text-slate-200'}`}>{file.original_name}</p>
        </div>
  ...
  return (
    <div
      ...
      className={`group relative rounded-xl border transition-all cursor-grab active:cursor-grabbing select-none overflow-hidden
        ${checked
          ? 'border-cyan-500/60 bg-cyan-500/12 ring-2 ring-cyan-500/30'
          : selected
    ...
    >
      ...
      <div className="px-3 py-2.5">
        <p className={`text-xs font-medium truncate leading-tight ${checked || selected ? 'text-cyan-200' : 'text-slate-200'}`}
